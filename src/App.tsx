...
function App() {
  ...
  useEffect(() => {
    refresh().catch((error) => setLog(error instanceof Error ? error.message : String(error)));
  }, []);
    ...
    <main className="shell">
      <aside className="sidebar">
        ...
        <div className="trust-note">
          ...
          <span>Plugins run inside Vencord. Install code you trust.</span>
        </div>
      </aside>
      ...
      <section className="content">
        <header className="topbar" id="dashboard">
          <div>
            ...
            <h2>Install plugins, rebuild Vencord, apply to Vesktop.</h2>
          </div>
        ...
        <section className="plugin-list">
          {manifest?.plugins.length ? (
            ...
            <div className="empty">No plugins managed yet. Add a local plugin or Git source to start.</div>
          )}
        </section>

        <section className="panel split" id="build">
          <div>
        ...
        <section className="panel split" id="settings">
          <div>
            <p className="eyebrow">Vesktop apply</p>
            <h3>Point Vesktop at the validated build</h3>
            <label>
              Vesktop state.json
              <input value={vesktopStatePath} onChange={(event) => setVesktopStatePath(event.currentTarget.value)} placeholder="Auto-detected or paste path" />
            </label>
            <p className="hint">Vesktop must be fully restarted after applying.</p>
          </div>
          <div className="action-stack">
            <label className="switch policy">
              ...
              />
              <span>Auto rebuild when updates are detected</span>
            </label>
            <button
              disabled={!!busy}
              onClick={() =>
